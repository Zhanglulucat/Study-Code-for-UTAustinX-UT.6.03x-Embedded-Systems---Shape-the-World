...
void WaitForASLow(void){
	...
	while(1){
		...
		if (SW1 == 0x00){									//If PF4 is low, return
			return;	
		}
	}
}
...
void WaitForASHigh(void){
	...
	while(1){
		SW1 = GPIO_PORTF_DATA_R & 0x10;  //PF4 into SW1
		if (SW1 == 0x10){									//If PF4 is high, return
			return;
		}
	}
}
...
// Notes:   friendly means it does not affect other bits in the port
void ClearVT(void){
	GPIO_PORTF_DATA_R &= ~0x02;			// Clear PF1 to low
}
...
// Notes:   friendly means it does not affect other bits in the port
void SetReady(void){
	GPIO_PORTF_DATA_R |= 0x08;			// Set PF3 to high
}
...
// Notes:   friendly means it does not affect other bits in the port
void ClearReady(void){
	GPIO_PORTF_DATA_R &= ~0x08;			// Clear PF3 to low
}
...
void Delay1ms(unsigned long msec){
  unsigned long i;
  while(msec > 0){
    //i = 13333;  // Delay in simulation; just divided the 100ms example by 100
		i = (16000*25/30); //Delay on real board
    while(i > 0){
      i = i - 1;
    }
