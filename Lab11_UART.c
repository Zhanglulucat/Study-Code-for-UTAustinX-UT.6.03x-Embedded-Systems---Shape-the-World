...
// as part of Lab 11 implement this function
	...
	while (buffer[i] != 0) {	//Decimal 0 means the end of string
		UART_OutChar(buffer[i]);	//output char one by one
		i++;
	}
}
...
// as part of Lab 11 implement this function
	signed long i;	//index
	if (n <= 9999){	//if n<=9999, convert as requirement
		if (n == 0){	//decide string[3] depends on n == 0 or not
			String[3] = 0x30;
		} else {
			String[3] = n%10 + 0x30;
		}
		n = n/10;	//move to ten digit
		for (i = 2; i >= 0; i--){	//decide string[0-2]
			if (n != 0){
				String[i] = n%10 + 0x30;	//a decimal's ASCII code is decimal value plus 0x30.
			} else {
				String[i] = 0x20;	//0x20 means space in ASCII code
			}
			n = n/10;
		}
	} else {	//if n>9999, then convert to "****"
			for (i = 0; i <= 3; i++){
				String[i] = '*';
			}
	}
	String[4] = 0x20;
	String[5] = 0;	//decimal zero means null, string termination
}
...
// as part of Lab 11 implement this function
	...
	String[6] = 'c';
	String[7] = 'm';
	String[8] = 0;	//decimal zero means null, string termination
}
